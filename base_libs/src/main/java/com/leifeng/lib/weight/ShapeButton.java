...
import com.leifeng.lib.utils.FontUtils;
...
public class ShapeButton extends LinearLayout {
    ...
    private int mDefaultTextSize = FontUtils.spToPx(getContext(), 15);
    private int mDefaultTextGravity = 0x11; // Gravity.CENTER
    private String mText = null;

    // # Icon Attributes
    private Drawable mIconResource = null;
    private int mFontIconSize = FontUtils.spToPx(getContext(), 15);
    ...
    public ShapeButton(Context context) {
        ...
        mTextTypeFace = FontUtils.findFont(mContext, mDefaultTextFont, null);
        mIconTypeFace = FontUtils.findFont(mContext, mDefaultIconFont, null);
    ...
    public ShapeButton(Context context, AttributeSet attrs) {
        ...
        TypedArray attrsArray = context.obtainStyledAttributes(attrs, R.styleable.ShapeButtonsAttrs, 0, 0);
    ...
    private TextView setupTextView() {
        ...
        textView.setTextSize(FontUtils.pxToSp(getContext(), mDefaultTextSize));
    ...
    private TextView setupFontIconView() {
        ...
        if (mFontIcon != null) {
            ...
            if (!isInEditMode()) {
                fontIconView.setTextSize(FontUtils.pxToSp(getContext(), mFontIconSize));
    ...
    private void initAttributesArray(TypedArray attrsArray) {
        ...
        mDefaultBackgroundColor = attrsArray.getColor(R.styleable.ShapeButtonsAttrs_fb_defaultColor, mDefaultBackgroundColor);
        mFocusBackgroundColor = attrsArray.getColor(R.styleable.ShapeButtonsAttrs_fb_focusColor, mFocusBackgroundColor);
        mDisabledBackgroundColor = attrsArray.getColor(R.styleable.ShapeButtonsAttrs_fb_disabledColor, mDisabledBackgroundColor);

        mEnabled = attrsArray.getBoolean(R.styleable.ShapeButtonsAttrs_android_enabled, true);

        mDisabledTextColor = attrsArray.getColor(R.styleable.ShapeButtonsAttrs_fb_disabledTextColor, mDisabledTextColor);
        mDisabledBorderColor = attrsArray.getColor(R.styleable.ShapeButtonsAttrs_fb_disabledBorderColor, mDisabledBorderColor);
        mDefaultTextColor = attrsArray.getColor(R.styleable.ShapeButtonsAttrs_fb_textColor, mDefaultTextColor);
        // if default color is set then the icon's color is the same (the default for icon's color)
        mDefaultIconColor = attrsArray.getColor(R.styleable.ShapeButtonsAttrs_fb_iconColor, mDefaultTextColor);

        mDefaultTextSize = (int) attrsArray.getDimension(R.styleable.ShapeButtonsAttrs_fb_textSize, mDefaultTextSize);
        mDefaultTextSize = (int) attrsArray.getDimension(R.styleable.ShapeButtonsAttrs_android_textSize, mDefaultTextSize);

        mDefaultTextGravity = attrsArray.getInt(R.styleable.ShapeButtonsAttrs_fb_textGravity, mDefaultTextGravity);

        mBorderColor = attrsArray.getColor(R.styleable.ShapeButtonsAttrs_fb_borderColor, mBorderColor);
        mBorderWidth = (int) attrsArray.getDimension(R.styleable.ShapeButtonsAttrs_fb_borderWidth, mBorderWidth);

        mRadius = (int) attrsArray.getDimension(R.styleable.ShapeButtonsAttrs_fb_radius, mRadius);

        mRadiusTopLeft = (int) attrsArray.getDimension(R.styleable.ShapeButtonsAttrs_fb_radiusTopLeft, mRadius);
        mRadiusTopRight = (int) attrsArray.getDimension(R.styleable.ShapeButtonsAttrs_fb_radiusTopRight, mRadius);
        mRadiusBottomLeft = (int) attrsArray.getDimension(R.styleable.ShapeButtonsAttrs_fb_radiusBottomLeft, mRadius);
        mRadiusBottomRight = (int) attrsArray.getDimension(R.styleable.ShapeButtonsAttrs_fb_radiusBottomRight, mRadius);

        mFontIconSize = (int) attrsArray.getDimension(R.styleable.ShapeButtonsAttrs_fb_fontIconSize, mFontIconSize);

        mIconPaddingLeft = (int) attrsArray.getDimension(R.styleable.ShapeButtonsAttrs_fb_iconPaddingLeft, mIconPaddingLeft);
        mIconPaddingRight = (int) attrsArray.getDimension(R.styleable.ShapeButtonsAttrs_fb_iconPaddingRight, mIconPaddingRight);
        mIconPaddingTop = (int) attrsArray.getDimension(R.styleable.ShapeButtonsAttrs_fb_iconPaddingTop, mIconPaddingTop);
        mIconPaddingBottom = (int) attrsArray.getDimension(R.styleable.ShapeButtonsAttrs_fb_iconPaddingBottom, mIconPaddingBottom);

        mTextAllCaps = attrsArray.getBoolean(R.styleable.ShapeButtonsAttrs_fb_textAllCaps, false);
        mTextAllCaps = attrsArray.getBoolean(R.styleable.ShapeButtonsAttrs_android_textAllCaps, false);

        mGhost = attrsArray.getBoolean(R.styleable.ShapeButtonsAttrs_fb_ghost, mGhost);
        mUseSystemFont = attrsArray.getBoolean(R.styleable.ShapeButtonsAttrs_fb_useSystemFont, mUseSystemFont);

        String text = attrsArray.getString(R.styleable.ShapeButtonsAttrs_fb_text);

        if (text == null) { //no fb_text attribute
            text = attrsArray.getString(R.styleable.ShapeButtonsAttrs_android_text);
        }

        mIconPosition = attrsArray.getInt(R.styleable.ShapeButtonsAttrs_fb_iconPosition, mIconPosition);

        textStyle = attrsArray.getInt(R.styleable.ShapeButtonsAttrs_android_textStyle, Typeface.NORMAL);

        String fontIcon = attrsArray.getString(R.styleable.ShapeButtonsAttrs_fb_fontIconResource);

        String iconFontFamily = attrsArray.getString(R.styleable.ShapeButtonsAttrs_fb_iconFont);
        String textFontFamily = attrsArray.getString(R.styleable.ShapeButtonsAttrs_fb_textFont);

        try {
            mIconResource = attrsArray.getDrawable(R.styleable.ShapeButtonsAttrs_fb_iconResource);
        } catch (Exception e) {
            mIconResource = null;
        }
        ...
        if (!isInEditMode()) {
            mIconTypeFace = iconFontFamily != null
                    ? FontUtils.findFont(mContext, iconFontFamily, mDefaultIconFont)
                    : FontUtils.findFont(mContext, mDefaultIconFont, null);

            mTextTypeFace = textFontFamily != null
                    ? FontUtils.findFont(mContext, textFontFamily, mDefaultTextFont)
                    : FontUtils.findFont(mContext, mDefaultTextFont, null);
        }
    }
    ...
    public void setTextSize(int textSize) {
        this.mDefaultTextSize = FontUtils.spToPx(getContext(), textSize);
        if (mTextView != null)
    ...
    public void setFontIconSize(int iconSize) {
        this.mFontIconSize = FontUtils.spToPx(getContext(), iconSize);
        if (mFontIconView != null)
    ...
    public void setCustomTextFont(String fontName) {
        mTextTypeFace = FontUtils.findFont(mContext, fontName, mDefaultTextFont);
    ...
    public void setCustomIconFont(String fontName) {
        ...
        mIconTypeFace = FontUtils.findFont(mContext, fontName, mDefaultIconFont);
